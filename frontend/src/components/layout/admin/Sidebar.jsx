...
import { LayoutDashboard, UserCheck, ShieldAlert, ShoppingBag, Percent, AlertCircle, BarChart3, Users, X } from 'lucide-react';

const DRAWER_WIDTH = 260;

const Sidebar = ({ mobileOpen, onMobileClose, isMobile }) => {
  const menuItems = [
    { text: 'Overview', path: '/admin', icon: LayoutDashboard },
    { text: 'Vendor Approvals', path: '/admin/vendor-approvals', icon: UserCheck },
    { text: 'Product Moderation', path: '/admin/product-moderation', icon: ShieldAlert },
    { text: 'Orders', path: '/admin/orders', icon: ShoppingBag },
    { text: 'Commission Rules', path: '/admin/commission-rules', icon: Percent },
    { text: 'Disputes', path: '/admin/disputes', icon: AlertCircle },
    { text: 'Reports', path: '/admin/reports', icon: BarChart3 },
  ...
  const drawerContent = (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', bgcolor: '#0F172A', color: '#FFFFFF' }}>
      ...
      <Box sx={{ p: 2.5, display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
        <Box>
          <Typography variant="h5" fontWeight={900} color="#38BDF8" sx={{ letterSpacing: '-0.04em' }}>
            Vendra
          </Typography>
